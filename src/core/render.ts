...
/** Downscaled data-URL for thumbnails (same pixels as PDF, fewer bytes). */
export function canvasToPreviewUrl(canvas: HTMLCanvasElement, maxWidth = 320): string {
  const ratio = Math.min(1, maxWidth / canvas.width);
  if (ratio >= 1) return canvas.toDataURL("image/jpeg", 0.7);
